...
class HTMLReportCreator(ReportCreator):
	def create(self, data, path):
		...
		f = open(path + "/STATS%s.txt" % (data.projectname), 'w')
		format = '%Y-%m-%d %H:%M:%S'
		# self.printHeader(f)

		f.write('ID = %s' % data.projectname)

		# self.printNav(f)

		f.write('\n')
		#f.write('Project name\' % (data.projectname))
		f.write('Generated %s (in %d seconds)' % (datetime.datetime.now().strftime(format), time.time() - data.getStampCreated()))
		f.write('\n')
		# f.write('<dt>Generator</dt><dd><a href="http://gitstats.sourceforge.net/">GitStats</a> (version %s), %s,' % (getversion(), getgitversion()))
		f.write('Report Period %s to %s' % (data.getFirstCommitDate().strftime(format), data.getLastCommitDate().strftime(format)))
		f.write('\n')
		f.write('Age %d days' % (data.getCommitDeltaDays()))
		f.write('\n')
		f.write('%d Active Days' % len(data.getActiveDays()))
		f.write('\n')
		##f.write('%3.2f%% Active Days Percentage' % (100.0 * len(data.getActiveDays()) / data.getCommitDeltaDays())) 
		f.write('Total Files %s ' % data.getTotalFiles())
		f.write('\n')
		f.write('Total Lines of Code %s ' % (data.getTotalLOC()))
		f.write('\n')
		f.write('Total Commits %s ' % (data.getTotalCommits()))
		f.write('\n')
		f.write('%s Authors' % (data.getTotalAuthors()))
		f.write('\n')
		...
		f.write('\n')	
		f.write('Commits by HOUR OF THE DAY (0-23)')
		f.write('\n')
		for i in range(0, 24):
			if i in hour_of_day:
				f.write('%d %d\n' % (i, hour_of_day[i]))
		...
		# Hour of Week
#!!!!!!!!!
		f.write('Weekday / Commits BY hour \n')
		...
		# Month of Year
		f.write('\n')
		f.write('Commits by Month of Year\n')
		...
		f.write('\n')
		f.write('Commits by year/month')
		f.write('\n')
		f.write('Month , Commits , Lines added , Lines removed \n')
